...
const configPath = "config/main_public_config.json"
...
const deploy = async () => {
    ...
    if (configContent[chainId].step < 2) {
        const TokenLockForPublic = await ethers.getContractFactory('TokenLockForPublic')
    ...
    if (configContent[chainId].step < 4) {
        const tokenLockForPublic = await ethers.getContractAt('TokenLockForPublic', configContent[chainId].tokenLockForPublic, wallet)
